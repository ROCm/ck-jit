...
def _entry_flags(entry):
    ...
    parts = []
    i = 1  # skip argv[0] (compiler binary)
    while i < len(argv):
        a = argv[i]
...
def generate_embedded_header(entries, embedded_h, is_fwd):
    ...
    with open(embedded_h, "w") as f:
        f.write(
            ...
            "#pragma once\n"
            "namespace ck_jit {\n"
            "struct BlobEntry { const char* name; const char* source; const char* flags; };\n"
            "static const BlobEntry kEntries[] = {\n"
        )
        for e in blob_entries:
            f.write(
                f"    {{ {json.dumps(os.path.basename(e['source']))}, "
                f"{json.dumps(e['source'])}, "
