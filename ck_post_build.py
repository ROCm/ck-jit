...
def quick_rebuild_lib(state_path, verbose=False, aiter_dir=""):
    ...
    # ---- Regenerate embedded manifest header if missing ----
    embedded_h = os.path.join(build_dir, "ck_jit_manifest_embedded.h")
    if not os.path.exists(embedded_h):
        ...
        if os.path.exists(manifest_base + ".ndjson"):
            ...
            n = generate_embedded_header(entries, embedded_h, is_fwd)
            print(f"{tag} Regenerated embedded manifest ({n} entries) → {embedded_h}",
                  file=sys.stderr)
        else:
            print(f"{tag} WARNING: manifest NDJSON not found; embedded header missing.",
                  file=sys.stderr)
